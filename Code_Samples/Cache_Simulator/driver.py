...
def main():
    ...
    print("{:<22}{:>8.1f}{:>10d}".format("Csim correctness", csim_cscore[0], 
                                         maxscore['csim']))

    misses = str(miss32)
    if miss32 == 2**31-1:
        misses = "invalid"
    print("{:<22}{:>8.1f}{:>10d}{:>12}".format("Trans perf 32x32", trans32_score, 
                                               maxscore['trans32'], misses))

    misses = str(miss64)
    if miss64 == 2**31-1:
        misses = "invalid"
    print("{:<22}{:>8.1f}{:>10d}{:>12}".format("Trans perf 64x64", trans64_score, 
                                               maxscore['trans64'], misses))

    misses = str(miss61)
    if miss61 == 2**31-1:
        misses = "invalid"
    print("{:<22}{:>8.1f}{:>10d}{:>12}".format("Trans perf 61x67", trans61_score, 
    ...
    if autograde:
        autoresult = "{:.1f}:{:d}:{:d}:{:d}".format(total_score, miss32, miss64, miss61)
